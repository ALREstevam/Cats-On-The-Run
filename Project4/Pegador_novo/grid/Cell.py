...
class Cell:
    ...
    # Construtor
    def __init__(self, x, y, grid, is_goal=False, is_wall=False, difficulty=0):
        '''
    ...
    def __getattr__(self, item):
        if item in ('neighbors', 'neib', 'neibs', 'neibors', 'neibours', 'vizinhos'):#se for feito algo como `cell.neibors`
            if self.__neighbors is None: #Caso os vizinhos da célula não tenham sido calculados
                self.add_neighbors(self.static_grid_ref)#Calcule os vizinhos
            return self.__neighbors #Retorne os vizinhos
        else:
            raise AttributeError('The following parameter was not found `' + item + '`')

    def position_is_valid(self, grid, row, col):
        '''
    ...
    def add_neighbors(self, grid):
        ...
        '''
        a variável `local_nodes` é um dicionário que deve ser preenchido da seguinte forma:
    ...
    def __str__(self):
        ...
        return '<:Cell {}; wall:{}; goal:{}; dif:{:.2f}; score:{:.2f}>'\
            .format(
                self.pos,
                'T' if self.is_wall else 'F',
                'T' if self.is_goal else 'F',
                self.difficulty,
                self.score
        )
    ...
    def direction_to(self, to):
        ...
        walk_direction = ''
        # North
        if self.xpos > to.xpos and self.ypos < to.ypos or self.ypos > to.ypos:
            walk_direction += 'N'
        # South
        elif  self.xpos <  to.xpos and self.ypos < to.ypos or self.ypos > to.ypo:
            walk_direction += 'S'
        # East
        if  self.ypos < to.ypos:
            walk_direction += 'E'
        # West
        elif self.ypos > to.ypos:
            walk_direction += 'W'
