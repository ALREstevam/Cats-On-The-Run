...
class Grid:
    ...
    def assign_cell(self, row : int, col : int):
    ...
    def redesign_reset_grid(self, goals, walls, start):
        self.goals = goals
        self.walls = walls
        self.start = start
        ...
        for row in self.grid:
            for elem in row:
                ...
                if elem.pos in walls:
                    elem.is_wall = True

                if elem.pos in goals:
                    elem.is_goal = True



        #self.redesign_grid_easy(goals, walls, start)


    def redesign_grid_easy(self, goals, walls, start):
        self.goals = goals
        self.walls = walls
        self.start = start

        for goal_pos in goals:
            self.get_cell(goal_pos).is_goal = True

        for block_pos in walls:
            self.get_cell(block_pos).is_wall = True
    ...
    def get_difficulties(self):
        ...
        for row in self.grid:
            for elem in row:
                answer.append(elem.difficulty)
        return answer

    def assign_difficulty(self, steps):
        if steps < 1:
            return
        # First Pass & assign
        for row in self.grid:
    ...
    def show_difficulties(self, show_colors = True):
        ...
        for row_index in range(self.rows):
            ...
            for col_index in range(self.cols):
                ...
                if exitsList is not None and (row_index, col_index) in exitsList:
                    answ.append(bcolors.OKGREEN + '[{:04.2f}]'.format(int(self.get_cell_at(row_index, col_index).difficulty)) + bcolors.ENDC)
                # Wall
                elif self.get_cell_at(row_index, col_index).is_wall:
                    answ.append('[' + (str(self.get_cell_at(row_index, col_index).show())*5) + ']')
                else:
                    answ.append('[{:04.2f}]'.format(int(self.get_cell_at(row_index, col_index).difficulty)))
            answ.append('\n')
        return ''.join(answ)
